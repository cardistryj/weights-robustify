...
from config import args_preactresnet18
from utils_dp import load_model, AverageMeter, accuracy
...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, transform):
        images = np.load(f'./data/data_{test_id}_{adv_num}_{offset}.npy')
        labels = np.load(f'./data/label_{test_id}_{adv_num}_{offset}.npy')
        assert labels.min() >= 0
...
def save_checkpoint(state, arch):
    filepath = os.path.join(arch + f'_{test_id}_{adv_num}_{offset}.pth.tar')
