...
def main():
    ...
    ori_acc, _ = test(ori_test_loader,net, 'testset')
    ...
    for ratio in np.linspace(0.08, 0.8, num = 10, endpoint=True):
        ...
        for _ in range(args.avg_runs):
            ...
            target_layer = prune.identity(get_last_conv(net_iter), 'weight')
            length = target_layer.out_channels
            mask: torch.Tensor = target_layer.weight_mask
            prune_num = int(length * args.prune_ratio)

            logger.info(f'{"="*20} Pruning {"="*20}')
            prune_step(t_loader, net_iter, mask, prune_num=max(prune_num - 10, 0))
            test(t_loader,net_iter, 'valid prune')

            for i in range(min(10, length)):
                print('Iter: ', i)
                prune_step(t_loader, net_iter, mask, prune_num=1)
                clean_acc, _ = test(t_loader,net_iter, 'valid prune')
                # 这里是 准确率 如果降得太多就停止
                if ori_acc - clean_acc > 20:
                    break

            prune.remove(get_last_conv(net_iter), 'weight')

            logger.info(f'{"="*20} Tuning {"="*20}')
            for epoch in range(20):
                train(t_loader,net_iter,"fine_pruning")
