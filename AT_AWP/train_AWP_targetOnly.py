...
from seam_utils import transform_test, transform_train, split_dataset, add_trigger_to_dataset, shuffle_label, get_target_dataset
...
def main():
    ...
    target_dataset = get_target_dataset(untrust_dataset, args.target_label_1)
    ...
    for epoch in range(args.epochs):
        logger.info(f'{"="*20} AWP Train {"="*20}')
        for batch, (target_x, target_y) in enumerate(tqdm(target_loader)):
            target_x, target_y = target_x.to(device), target_y.to(device)
            # enabling awp in trojan training
            if args.attack == 'pgd':
                # Random initialization
                delta = attack_pgd(net, target_x, target_y, epsilon, pgd_alpha, args.attack_iters, args.restarts, args.norm)
                delta = delta.detach()
            elif args.attack == 'fgsm':
                delta = attack_pgd(net, target_x, target_y, epsilon, args.fgsm_alpha*epsilon, 1, 1, args.norm)
            # Standard training
            elif args.attack == 'none':
                delta = torch.zeros_like(target_x)
            X_adv = normalize(torch.clamp(target_x + delta[:target_x.size(0)], min=lower_limit, max=upper_limit))
            ...
            robust_output = net(X_adv)
            loss = criterion(robust_output, target_y)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
